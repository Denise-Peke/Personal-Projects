...
//  slot-machine
//
//  Created by Denise on 26/06/2023.
//
...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .alert(item: $showingAlert){ alert -> Alert in
            switch alert {
                case .success:
                    return Alert(title: Text("Yeah!!! your won"), message: Text("Born with the charm"), dismissButton: .cancel())
                case .failure:
                    return Alert(title: Text("ups!"), message: Text("better luck next time"), dismissButton: .cancel())
            }
